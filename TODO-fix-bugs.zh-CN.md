...
- ⏳ 剩余：**2 项 P2 高层级**条目（路由相关，需运行应用验证）+ **7 项 P3**
  延期项（含延期理由）。
...
### 1. 「卡片挡住连线」存在两套互相分叉的路由器 —— *已确认*
- **位置：** `updateEdgeEndpoints.ts`（`liveRoute` 开关）
- **问题：** `detourRoute`（基于当前端口、绕障）仅在 `liveRoute` 为真时运行；而
  静态 / 初次布局 / SVG 导出走的是另一条路径——对接到被冻结的 dagre
  `dagreWaypoints`。同一个问题用两种方式解决、由一个会话布尔量切换，二者对同一
  布局可能呈现不同视觉效果，且每次改路由都得改两遍。这是「导出与画布路由不一致」
  的根因。
- **修复方向：** 始终走 `detourRoute`；把 dagre 降级为只做排序 / 摆位，废弃
  `dagreWaypoints` 分支。
- **风险：大改，暂不执行。** 视觉风险高（始终 detour vs 冻结 dagre 通道），需先做
  前后对比截图。
