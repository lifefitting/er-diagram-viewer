...
当前稳定基线：**v0.3.6**（发布标签 `V0.3.6`；大图性能优化、文字清晰度、三入口引导与自定义模块配色）。
...
- **模块与配色定制**：按 FK 邻接 + 表名前缀聚类业务模块，八套配色（沉稳 / 青瓷 / 暮光 / 高辨识 / 明快 / 柔和 / 大地 / 单色）可切换；多选表可创建自定义模块，模块色块支持预设、自选颜色及 HEX 输入，并标明色板来源
...
- **大图流畅交互**：画布平移、缩放和拖表时只更新必要的覆盖层几何与受影响连线，避免整棵字段树重复渲染；拖动、滚动、缩放、搜索跳转和模块定位期间在底部中央实时显示当前 FPS，停止后自动收起
...
- **明暗主题**：跟随系统 / 强制亮色 / 强制暗色
- **按需加载与性能门禁**：Cytoscape (~600 KB) 经 React.lazy + manualChunks 拆出独立 chunk；仓库内置可复现性能基准、交互 E2E 与 CI 性能预算
...
首次打开新的会话不会自动创建工作区。空画布会提供三个直接入口：点击 **查看示例 ER 图**
加载内置电商样例、拖入 SQL/DDL 文件（也支持直接拖入 SQL 文本），或拖入 `.erreview`
恢复已有评审现场。当前标签页已有 `sessionStorage` 工作区时仍会直接恢复，不显示或闪现新手引导。
...
1. 新会话可直接点击 **查看示例 ER 图**，或把 `.sql` / `.ddl` / `.txt` 拖到空画布；顶部
   **导入** 仍可打开完整编辑器，支持粘贴脚本，`⌘/Ctrl + Enter` 解析、`Esc` 关闭。
