# DB ER Diagram Viewer

把 `CREATE TABLE` / `ALTER TABLE` 脚本贴进浏览器，直接绘制出可交互的 ER 图，用于 review 数据库设计。即使脚本没写 `FOREIGN KEY`，也会按命名约定 + 类型 + 索引启发式地推断外键关系，并允许人工确认。
...
bun run build:single    # 单文件构建 → dist-single/index.html（双击即用）
bun run preview
```
...
| **单文件** | `bun run build:single` | `dist-single/index.html`（~900 KB，gzip ~290 KB） | **双击即用、邮件 / U 盘分发、`file://` 离线打开**——零依赖、零服务器 |
...
│   │   ├── ExportMenu.tsx
│   │   ├── SqlInputDialog.tsx
│   │   └── icons.tsx             # Toolbar / Dialog 共享 SVG 图标
│   ├── sidebar/                  # 左侧控制面板
│   │   ├── Sidebar.tsx           # 壳子（含折叠态导轨）
│   │   ├── CollapsedSidebarRail.tsx
│   │   ├── GroupHeading.tsx
│   │   ├── AccordionSection.tsx
│   │   ├── DisplayControls.tsx
│   │   ├── InferencePanel.tsx
│   │   ├── ModulesPanel.tsx
│   │   └── icons.tsx             # Sidebar 专用 glyph
│   └── theme/
│       └── useApplyTheme.ts      # 监听主题偏好并 toggle html.dark
└── exports/
